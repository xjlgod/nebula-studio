...
func ImportAction(taskID string, address string, username string, taskAction TaskAction) (result ActionResult, err error) {
	...
	switch taskAction {
	case ActionQuery:
		actionQuery(taskID, address, username, &result)
	case ActionQueryAll:
		actionQueryAll(address, username, &result)
	case ActionStop:
		actionStop(taskID, address, username, &result)
	case ActionStopAll:
...
func actionQuery(taskID string, address string, username string, result *ActionResult) {
...
func actionStop(taskID string, address string, username string, result *ActionResult) {
	if id, err := strconv.Atoi(taskID); err != nil {
		zap.L().Warn(fmt.Sprintf("stop task fail, id : %s", taskID), zap.Error(err))
		result.Msg = "Task not existed"
		return
